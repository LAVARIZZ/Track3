...
st.write("Welcome! We’re here to guide you in making informed investment choices based on your selected stocks and their financial potential.")
...
money = st.sidebar.number_input('Enter the amount in $', min_value=1, max_value=10000000, value=500)
...
with st.container():
    ...
    """.format(", ".join(assets)), unsafe_allow_html=True)
...
with st.container():
    ...
    """.format(percent_ret, percent_vols, percent_var), unsafe_allow_html=True)
...
with st.container():
    ...
    """.format(pd.DataFrame(list(cleaned_weights.items()), columns=['Stock', 'Weight']).to_html(index=False, classes='table table-bordered', border=0),
               pd.DataFrame(esg_sum).reset_index().rename(columns={0: 'Stock', 1: 'ESG Score Sum'}).to_html(index=False, classes='table table-bordered', border=0)),
...
with st.container():
    ...
    """.format(pd.DataFrame(list(allocation.items()), columns=['Stock', 'Quantity']).to_html(index=False, classes='table table-bordered', border=0)), unsafe_allow_html=True)
...
with st.container():
    ...
    """.format(round(leftover)), unsafe_allow_html=True)
